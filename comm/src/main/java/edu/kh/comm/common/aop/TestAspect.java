...
@Aspect // 공통 관심사(특정 흐름 사이에 끼어서 수행할 코드)가 작성된 클래스임을 명시
		// -> 해당 어노테이션이 작성된 클래스에는 
		//	advice, pointcut (advice가 끼어들어서 수행될 부분)이 작성되어 있어야 함
...
//	@Before() : Pointcut에 작성된 메서드가 수행되기 전 advice 수행을 명시하는 어노테이션
	//@Before("execution( * edu.kh.comm.member..*Impl.*(..) )")
//	@Before("CommonPointcut.implPointcut()")	//CommonPoint 메서드의 implPointcut() 메서드의 패턴
	public void start() { //서비스 코드 수행 전에 수행되는 메서드(advice)
		logger.info("-------------------------- Service Start -----------------------");
	}
	
//	@After() : Pointcut에 작성된 메서드가 수행된 후 advice 수행을 명시하는 어노테이션
	//@After("execution( * edu.kh.comm.member..*Impl.*(..) )")
//	@After("CommonPointcut.implPointcut()")
	public void end() { //서비스 코드 수행된 후 수행되는 메서드(advice)
